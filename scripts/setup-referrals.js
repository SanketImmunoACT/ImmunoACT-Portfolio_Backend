  ...
  try {
    ...
    const [stats] = await sequelize.query(`
      SELECT 
        ...
        SUM(CASE WHEN status = 'Converted to Job' THEN 1 ELSE 0 END) as convertedReferrals,
        SUM(CASE WHEN urgency IN ('High', 'Urgent') THEN 1 ELSE 0 END) as urgentReferrals
      FROM job_referrals
    `);
